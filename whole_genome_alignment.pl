...
my $bref_name = basename($ref_fa,".fa",".fasta");
my $btar_name = basename($tar_fa,".fa",".fasta");
...
### Alignment parameter ###
if($dist eq "self"){
	$chainPar = "-minScore=3000 -linearGap=$Bin/lastz_params/near.linearGap";
	$lastzPar = "O=400 E=30 X=910 Y=9400 K=3000 L=3000 H=2000 T=1 Z=1";
} elsif($dist eq "near"){
	$chainPar = "-minScore=5000 -linearGap=$Bin/lastz_params/near.linearGap";
	$lastzPar = "E=150 H=2000 K=4500 L=2200 M=254 O=600 Q=$Bin/lastz_params/near.q T=2 Y=15000";
} elsif($dist eq "medium"){
	$chainPar = "-minScore=3000 -linearGap=medium";
	$lastzPar = "E=30 H=2000 K=3000 L=2200 M=254 O=400 Q=$Bin/lastz_params/medium.q T=1 Y=9400";
} elsif($dist eq "far"){
	$chainPar = "-minScore=5000 -linearGap=loose";
	$lastzPar = "E=30 H=2000 K=2200 L=6000 M=50 O=400 Q=$Bin/lastz_params/far.q T=2 Y=3400";
}
...
foreach my $ref_file (@ref_fas) {
	my $ref_name = basename($ref_file,".fa",".fasta");
	foreach my $tar_file (@tar_fas) {
		my $tar_name = basename($tar_file,".fa",".fasta");
...
`$Bin/splitChainNet.pl --path $path_conf --chain $out_dir/all.chain --net $out_dir/all.net -o $chainNet_dir`;
